...
namespace CargaDataSetMurcia.Model
{
    public static class CargaRDF
    {
        ...
        public static void GenerarRDF(string pUrlUrisFactory)
        {
            ...
            GenerarOrganizaciones(outputFolder + "/Organizaciones/", pUrlUrisFactory, centros, departamentos, gruposInvestigacion, datoEquiposInvestigacion, proyectos, equiposProyectos, fechasEquiposProyectos, lineasInvestigador, lineasDeInvestigacion, lineasUnesco,codigosUnesco);
            GenerarOrganizacionesExternas(outputFolder + "/OrganizacionesExternas/", pUrlUrisFactory, organizacionesExternas);
            GenerarFuentesFinanciacion(outputFolder + "/FuentesFinanciacion/", pUrlUrisFactory, fuentesFinanciacionProyectos);
            GenerarProyectos(outputFolder + "/Proyectos/", pUrlUrisFactory, proyectos, fechasProyectos, equiposProyectos, fechasEquiposProyectos, personas, features, areasUnescoProyectos,codigosUnesco, organizacionesExternas, fuentesFinanciacionProyectos);
        ...
        private static void GenerarPersonas(string pRuta, string pUriUrisFactory, List<Persona> pPersonas, List<Centro> pCentros, List<Departamento> pDepartamento, List<DatoEquipoInvestigacion> pDatoEquipoInvestigacion, List<TipoParticipacionGrupo> pTipoParticipacionGrupo, List<GrupoInvestigacion> pGrupoInvestigacion, List<Patentes> pPatentes, List<InventoresPatentes> pInventoresPatentes, List<EquipoProyecto> pEquipoProyecto, List<FechaEquipoProyecto> pFechaEquipoProyecto)
        {
            ...
            foreach (Persona persona in pPersonas)
            {
                ...
                if (persona.SEXO == "M")
                {
                    INode uriPredicateFoafGender = graph.CreateUriNode(new Uri("http://purl.org/roh/mirror/foaf#gender"));
                    INode objectFoafGender = CreateTextNode(graph, "male");
                    Triple tripleGender = new Triple(uriSubject, uriPredicateFoafGender, objectFoafGender);
                    graph.Assert(tripleGender);
                }else if (persona.SEXO == "F")
                {
                    INode uriPredicateFoafGender = graph.CreateUriNode(new Uri("http://purl.org/roh/mirror/foaf#gender"));
                    INode objectFoafGender = CreateTextNode(graph, "female");
                    Triple tripleGender = new Triple(uriSubject, uriPredicateFoafGender, objectFoafGender);
                    graph.Assert(tripleGender);
                }
        ...
        private static void GenerarArticulos(string pRuta, string pUriUrisFactory, List<Articulo> pArticulos, List<Persona> pPersonas, List<AutorArticulo> pAutorArticulo, List<PalabrasClaveArticulos> pPalabrasClave)
        {
            ...
            foreach (Articulo articulo in pArticulos)
            {
                ...
                freetextsearch += " " + articulo.TITULO.ToLower();
                ...
                foreach (PalabrasClaveArticulos palabraClave in palabrasClave)
                {
                    ...
                    freetextsearch += " " + palabraClave.PC_PALABRA.ToLower();
                }
                ...
                if (!string.IsNullOrEmpty(articulo.AREA) && !string.IsNullOrEmpty(articulo.DESCRI_AREA))
                {
                    ...
                    freetextsearch += " " + articulo.DESCRI_AREA.ToLower();
                }
                ...
                if (autorArticuloID.ContainsKey(articulo.CODIGO))
                {
                    ...
                    {
                        if (pPersonas.Exists(x => x.IDPERSONA == autorArticulo.IDPERSONA))
                        {
                ...
                //freetextsearch
                INode uriPredicateFreetextSearch = graph.CreateUriNode(new Uri("http://purl.org/roh#freetextSearch"));
                INode uriObjectFreetextSearch = CreateTextNode(graph, freetextsearch);
                Triple tripleFreetextSearch = new Triple(uriSubject, uriPredicateFreetextSearch, uriObjectFreetextSearch);
                graph.Assert(tripleFreetextSearch);
        ...
        private static void GenerarOrganizaciones(string pRuta, string pUrlUrisFactory, List<Centro> pCentros, List<Departamento> pDepartamentos, List<GrupoInvestigacion> pGruposInvestigacion, List<DatoEquipoInvestigacion> pDatoEquipoInvestigacions, List<Proyecto> pProyecto, List<EquipoProyecto> pEquipoProyecto, List<FechaEquipoProyecto> pFechaEquipoProyectos, List<LineasInvestigador> pLineasInvestigador, List<LineasDeInvestigacion> pLineasInvestigacion, List<LineasUnesco> pLineasUnesco,List<CodigosUnesco> pCodigosUnesco)
        {
            ...
            foreach (GrupoInvestigacion grupoInvestigacion in pGruposInvestigacion)
            {
                ...
                Triple tripleTitle = new Triple(uriSubject, uriPredicateRohTitle, objectRohTitle);
                freetextsearch += " " + grupoInvestigacion.DESCRIPCION.ToLower();
                ...
                HashSet<string> aniadido = new HashSet<string>();
                    ...
                    {                        
                        foreach (LineasUnesco lineaUnesco in lineasUnesco)
                        {
                            ...
                            if (!string.IsNullOrEmpty(lineaUnesco.LIUN_UNAR_CODIGO) && lineaUnesco.LIUN_UNAR_CODIGO != "00")
                            {
                                ...
                                nivel1 = lineaUnesco.LIUN_UNAR_CODIGO;
                            }
                            if (!string.IsNullOrEmpty(lineaUnesco.LIUN_UNCA_CODIGO) && lineaUnesco.LIUN_UNCA_CODIGO != "00")
                            {
                                ...
                                nivel2 = lineaUnesco.LIUN_UNCA_CODIGO;
                            }
                            if (!string.IsNullOrEmpty(lineaUnesco.LIUN_UNES_CODIGO) && lineaUnesco.LIUN_UNES_CODIGO != "00")
                            {
                                ...
                                nivel3 = lineaUnesco.LIUN_UNES_CODIGO;
                            }
                            if (aniadido.Add(nivel1 + nivel2 + nivel3))
                            {
                                INode uriPredicateHasKnowledgeArea = graph.CreateUriNode(new Uri("http://purl.org/roh#hasKnowledgeArea"));
                                ...
                                freetextsearch += " " + pCodigosUnesco.First(x => x.UNES_UNAR_CODIGO == nivel1 && x.UNES_UNCA_CODIGO == nivel2 && x.UNES_CODIGO == nivel3).UNES_NOMBRE.ToLower();
                            }
                        }
                    }
                }
        ...
        private static void GenerarOrganizacionesExternas(string pRuta, string pUrlUrisFactory, List<OrganizacionesExternas> pOrganizacionesExternas)
        {
            ...
            foreach (OrganizacionesExternas organizacion in pOrganizacionesExternas)
            {
                ...
                foreach (OrganizacionesExternas organizacion2 in pOrganizacionesExternas.Where(x => x.ENTIDAD == organizacion.ENTIDAD))
                {
                    INode uriSubjectProyecto = GetUri(pUrlUrisFactory, graph, "http://purl.org/roh/mirror/vivo#Project", organizacion2.IDPROYECTO);
        ...
        private static void GenerarFuentesFinanciacion(string pRuta, string pUrlUrisFactory, List<FuentesFinanciacionProyectos> pFuentesFinanciacionProyectos)
        {
            System.IO.Directory.CreateDirectory(pRuta);
            int i = 0;
            int total = pFuentesFinanciacionProyectos.Select(x => x.AYFI_FUEN_CODIGO).Distinct().Count();
            ...
            foreach (FuentesFinanciacionProyectos fuentesFinanciacionProyectos in pFuentesFinanciacionProyectos)
            {
                ...
                if (fuentesFinanciacionProyectos.AYFI_FUEN_CODIGO!="COOR" && ids.Add(fuentesFinanciacionProyectos.AYFI_FUEN_CODIGO))
                {
                    ...
                    INode uriSubjectFuenteFinanciacion = GetUri(pUrlUrisFactory, graph, "http://purl.org/roh#FundingProgram", fuentesFinanciacionProyectos.AYFI_FUEN_CODIGO);
                    INode uriPredicateRdfType = graph.CreateUriNode(new Uri("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"));
                    INode uriObjectRdfTypeFuenteFinanciacion = graph.CreateUriNode(new Uri("http://purl.org/roh#FundingProgram"));
                    Triple tripleRdfTypeFuenteFinanciacion = new Triple(uriSubjectFuenteFinanciacion, uriPredicateRdfType, uriObjectRdfTypeFuenteFinanciacion);
                    graph.Assert(tripleRdfTypeFuenteFinanciacion);

                    //Título
                    INode uriPredicateRohTitle = graph.CreateUriNode(new Uri("http://purl.org/roh#title"));
                    INode objectRohTitle = CreateTextNode(graph, fuentesFinanciacionProyectos.FUEN_NOMBRE);
                    Triple tripleTitle = new Triple(uriSubjectFuenteFinanciacion, uriPredicateRohTitle, objectRohTitle);
                    graph.Assert(tripleTitle);

                    graph.SaveToFile(rutaFuenteFinanciacion);
                }
            }
            Console.WriteLine($"\rGenerando RDFs en {pRuta} {i}/{total}");
        }
        ...
        private static void GenerarProyectos(string pRuta, string pUrlUrisFactory, List<Proyecto> pProyectos, List<FechaProyecto> pFechaProyecto, List<EquipoProyecto> pEquipoProyecto, List<FechaEquipoProyecto> pFechaEquipoProyecto, List<Persona> pPersonas, List<Feature> pFeatures, List<AreasUnescoProyectos> pAreasUnescoProyectos, List<CodigosUnesco> pCodigosUnesco, List<OrganizacionesExternas> pOrganizacionesExternas,List<FuentesFinanciacionProyectos> pFuentesFinanciacionProyectos)
        {
            ...
            foreach (Proyecto proyecto in pProyectos)
            {
                ...
                Triple tripleTitle = new Triple(uriSubject, uriPredicateRohTitle, objectRohTitle);
                graph.Assert(tripleTitle);
                freetextsearch += " " + proyecto.NOMBRE.ToLower();
                ...
                FechaProyecto fechaProyectoEstado = pFechaProyecto.FirstOrDefault(x => x.IDPROYECTO == proyecto.IDPROYECTO);
                if (fechaProyectoEstado != null)
                {
        ...
        private static INode GetUri(string pUriUrisFactory, Graph pGraph, string pClass, string pID)
        {
            ...
            while (error)
            {
                try
                {
                    WebRequest request = WebRequest.Create($"{pUriUrisFactory}?resource_class={HttpUtility.UrlEncode(pClass)}&identifier={HttpUtility.UrlEncode(pID)}");
