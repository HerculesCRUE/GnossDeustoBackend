...
Detalle del proceso para foat#Person. Investigadores de un CV
--------------------
...
Entre otras configuraciones, por determinar, para el caso de los investigadores habrá al menos dos configuraciones para la similitud: 
1. Similitud por nombre: A través de un algoritmo, por determinar, se obtendrá la similitud (un valor entre 0% y 100%) entre el nombre del investigador a cargar y el de otros investigadores cargados. Estará establecido un valor mínimo (p.e. 70%) que indicará que es probable que se trate de la misma entidad pero que habrá que reforzar con el descubrimiento para poder llegar a la conclusión de que se trata de la misma entidad y un valor máximo (p.e. 90%) que implicará que se trata de la misma entidad. 
2. Similitud por nombre + paper: Se utilizará un algoritmo que obtenga la similitud teniendo en cuenta el nombre del investigador junto con el nombre de los papers, de forma que, aunque en el apartado anterior ‘similitud por nombre’ no se alcanza el umbral mínimo o máximo, se utilizará el refuerzo de los papers para detectar la similitud entre los investigadores. 
...
2. En segundo lugar, se intenta la reconciliación en base a similitudes entre la entidad a cargar y las entidades cargadas en ASIO. Para ello se buscan candidatos dentro del grafo de ASIO de la universidad cuyo nombre sea igual o similar al investigador que estamos intentado cargar. 
    1. En caso de encontrar un investigador cuyo nombre sea igual o su similitud supere el umbral máximo de similitud se considera que es la misma entidad por lo que terminamos la reconciliación. 
    2. En caso de encontrar más de investigador con exactamente el mismo nombre o algún investigador con un nombre cuya similitud con el nombre a cargar no supere el umbral máximo, pero sí que supere el umbral mínimo de similitud intentamos la reconciliación a través de la similitud combinada de nombre + papers. Es decir, obtenemos los nombres de los papers de las entidades candidatas y una vez obtenidos obtenemos la similitud entre la combinación nombre + paper de las entidades cargadas en ASIO con la entidad a cargar. 
        1. En caso de que la similitud de nombre + papers entre más de una entidad cargada y la entidad a cargar supere el umbral máximo no podemos reconciliar la entidad automáticamente. 
        2. En caso de que la similitud de nombre + papers entre una entidad cargada y la entidad a cargar supere el umbral máximo se considera que es la misma entidad por lo que terminamos la reconciliación. 
        3. En caso negativo se procede a intentar la reconciliación a través del descubrimiento. 
