...
namespace CronConfigure.Models.Services
{
    public class RepositoryCronService: IRepositoryCronService
    {
        ...
        public List<JobViewModel> GetJobs(Guid repositoryID)
        {
            ...
            {
                jobs.Add(_cronApiService.GetJob(id));
            }
            return jobs.OrderBy(item => item.ExecutedAt).ToList();
        }
        ...
        public List<JobViewModel> GetAllJobs(Guid repositoryID)
        {
            ...
            listAllJobs = listJobs.Union(listJobsOfRecurring).OrderBy(item => item.ExecutedAt).ToList();
            return listAllJobs;
        }
    }
}
